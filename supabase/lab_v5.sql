...
INSERT INTO lab_fiche_meta (name_vi, name_en, category, image_url, b2c_product_id, b2c_sku_ref, is_active)
SELECT p.name_vi, p.name_en, c.name_en, p.main_image_url, p.id, p.sku, true
FROM products p LEFT JOIN categories c ON c.id = p.category_id WHERE p.is_active = true;

INSERT INTO lab_fiche_variants (fiche_id, label, sku, is_default, sort_order)
SELECT m.id, 'Standard', m.b2c_sku_ref, true, 0 FROM lab_fiche_meta m;

UPDATE lab_assignments la SET fiche_id = m.id
FROM lab_fiche_meta m WHERE la.product_id = m.b2c_product_id AND la.fiche_id IS NULL;

UPDATE lab_assignments la SET variant_id = v.id
FROM lab_fiche_variants v
WHERE v.fiche_id = la.fiche_id AND v.is_default = true AND la.variant_id IS NULL AND la.fiche_id IS NOT NULL;

UPDATE lab_order_lines lo SET fiche_id = v.fiche_id, variant_id = v.id
FROM lab_fiche_variants v WHERE lo.product_sku = v.sku AND lo.fiche_id IS NULL;
...
-- Note: lab_profiles.id = auth.uid() (no separate user_id column)
CREATE POLICY "lab_fiche_meta_insert" ON lab_fiche_meta FOR INSERT TO authenticated WITH CHECK (
  (SELECT role FROM profiles WHERE id = auth.uid()) IN ('admin', 'lab_manager')
  OR (SELECT team FROM lab_profiles WHERE id = auth.uid()) = ANY(teams)
);
CREATE POLICY "lab_fiche_meta_update" ON lab_fiche_meta FOR UPDATE TO authenticated USING (
  (SELECT role FROM profiles WHERE id = auth.uid()) IN ('admin', 'lab_manager')
  OR (SELECT team FROM lab_profiles WHERE id = auth.uid()) = ANY(teams)
);
