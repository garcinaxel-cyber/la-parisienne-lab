      ...
      {byTeam.length > 0 ? (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {byTeam.map(({ team, items }) => {
            ...
            const done = items.filter((a: any) => a.status === 'done').length;
            return (
              <div key={team} className="card overflow-hidden">
                ...
                <div className="divide-y divide-border-soft">
                    ...
                    return (
                      <div key={a.id} className="flex items-center justify-between px-4 py-2.5">
                        ...
                        <div className="flex items-center gap-3 ml-3 shrink-0">
                          <span className="text-sm font-bold text-navy">×{a.total_qty}</span>
                  ...
                  {items.length > 6 && (
                    <div className="px-4 py-2 text-xs text-ink-light">+{items.length - 6} {lang === 'vi' ? 'khác' : 'more'}</div>
                  )}
                </div>
              </div>
            );
