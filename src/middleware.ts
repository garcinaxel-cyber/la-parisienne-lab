...
export function middleware(req: NextRequest) {
  ...
  // Inter-shop transfer 24h reminder cron (2026-09-07), secret-gated itself.
  if (pathname.startsWith('/api/odoo/shop-transfer-reminder')) return NextResponse.next();
  ...
  // Temporary read-only duplicate-transfer verification (2026-09-08), secret-gated. Remove after use.
  if (pathname.startsWith('/api/admin/dup-verify-debug')) return NextResponse.next();
