...
export async function GET(req: Request) {
  ...
  const { data: pending, error: pendingErr, count: pendingCount } = await supabase
  ...
  return NextResponse.json({ processed: results.length, results, _debug: { pendingErr: pendingErr ? String(pendingErr.message ?? pendingErr) : null, pendingCount, pendingLen: pending?.length ?? 0 } });
}
