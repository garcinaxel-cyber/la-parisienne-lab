          ...
          <div key={e.id} className="flex items-center gap-2.5 px-4 py-2.5 text-sm" style={{ borderTop: i === 0 ? 'none' : '1px solid #F3F4F6' }}>
            ...
            {e.qrCodeUrl ? (
              <div className="relative shrink-0">
                <img src={e.qrCodeUrl} alt="QR chuyển khoản" className="w-9 h-9 rounded-md object-cover" style={{ border: '1px solid #E5E7EB' }} />
                <button onClick={() => removeQr(e.id)} disabled={uploadingQrFor === e.id}
                  title="Xoá QR chuyển khoản" aria-label="Xoá QR chuyển khoản"
              ...
              <label title="Tải QR chuyển khoản cho event này" className="shrink-0 cursor-pointer inline-flex items-center gap-1 text-xs font-bold rounded-lg px-2.5 py-1.5"
