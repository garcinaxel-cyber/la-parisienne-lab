...
export default async function ReceptionPage() {
  ...
  // Pending transfer notes, newest first, with their lines
  ...
  const { data: lines } = ids.length
    ? await supabase.from('lab_stock_transfer_lines')
        .select('id, transfer_id, product_name_vi, product_name_en, sku, variant_label, image_url, qty_sent, qty_received')
        .in('transfer_id', ids)
    : { data: [] as any[] };
  ...
  const bons = (transfers ?? []).map(t => ({ ...t, lines: linesByTransfer[t.id] ?? [] }));

  return <ReceptionView bons={bons} />;
}
