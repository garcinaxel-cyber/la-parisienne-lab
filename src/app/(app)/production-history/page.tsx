...
export default async function ProductionHistoryPage() {
  ...
  const { data: rows } = await supabase
    ...
    .select('qty_produced, total_qty, cancelled, is_extra, produced_at, team, produced_ahead, transferred, product_name_vi, lab_imports!inner(delivery_date, status)')
  ...
  // Backlog: production that was DUE (not made-ahead) and marked "done", but never handed off
  // to stock at all — invisible until now (the existing reminder only ever looked at TODAY's
  // Odoo sync, never at whether a card was sent to stock in the first place). Found via a live
  // data audit on 2026-07-27: ~1800 pieces across all 4 teams, some over a week old.
  const byBacklogDay = new Map<string, { date: string; pieces: number; cards: number; teams: Set<string> }>();
  let backlogTotalQty = 0, backlogTotalCards = 0;
  for (const a of rows ?? []) {
    ...
    const deliveryDate = (a as any).lab_imports?.delivery_date as string | undefined;
    const isDue = !!deliveryDate && deliveryDate <= today;
    if (!a.transferred && !a.produced_ahead && isDue) {
      const bc = byBacklogDay.get(date) ?? { date, pieces: 0, cards: 0, teams: new Set<string>() };
      bc.pieces += qty; bc.cards += 1; if (a.team) bc.teams.add(a.team);
      byBacklogDay.set(date, bc);
      backlogTotalQty += qty; backlogTotalCards += 1;
    }
  }

  const days = Array.from(byDay.values()).sort((a, b) => b.date.localeCompare(a.date));
  const backlogDays = Array.from(byBacklogDay.values())
    .map(b => ({ date: b.date, pieces: b.pieces, cards: b.cards, teams: Array.from(b.teams) }))
    .sort((a, b) => a.date.localeCompare(b.date));

  return <ProductionHistoryView days={days} today={today}
    backlog={{ totalQty: backlogTotalQty, totalCards: backlogTotalCards, days: backlogDays }} />;
}
